(ns status-im.chat.events
  (:require [re-frame.core :as re-frame]
            ...
            [status-im.utils.gfycat.core :as gfycat-utils]
...
(re-frame/reg-cofx
  :gfy-generator
  (fn [cofx _]
    (assoc cofx :gfy-generator gfycat-utils/generate-gfy)))
...
;; TODO(janherich); get rid of this hacky loaded-event once we remove old webview wallet (the only place where it's used)
(handlers/register-handler-db
  :add-chat-loaded-event
  [re-frame/trim-v]
  (fn [db [chat-id event]]
    (assoc-in db [:chat-loaded-event chat-id] event)))

(handlers/register-handler-fx
  :invoke-chat-loaded-event
  [re-frame/trim-v]
  (fn [{:keys [db]} [chat-id]]
    (when-let [event (get-in db [:chat-loaded-event chat-id])]
      {:dispatch event
       :db       (assoc db :chat-loaded-event nil)})))
